...
COCO_NAMES = ['atypical', 'indeterminate', 'negative','typical', '__background__']
...
class RSNA_COCO(data.Dataset):
  def __init__(self, data_dir, split, split_ratio=1.0, img_size=512):
    ...
    if split == 'test':
      self.annot_path = os.path.join(self.data_dir, 'labels.json')
    else:
      self.annot_path = os.path.join(self.data_dir, 'labels.json')
...
class RSNACOCO_eval(RSNA_COCO):
  def __init__(self, data_dir, split, test_scales=(1,), test_flip=False, fix_size=False):
    super(RSNACOCO_eval, self).__init__(data_dir, split)
...
if __name__ == '__main__':
  ...
  dataset = RSNA_COCO('E:\\coco_debug', 'train')
  for d in dataset:
    b1 = d
  #   pass
